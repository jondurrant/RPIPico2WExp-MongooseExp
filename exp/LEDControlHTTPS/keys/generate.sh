...
openssl req -x509 -new -key ca.key -days 3650 -subj /CN=test.drjonea.co.uk -out ca.crt

# Generate server cert
openssl ecparam -noout -name prime256v1 -genkey -out server.key
openssl req -new -sha256 -key server.key -days 3650 -subj /CN=pico2w-web.local.jondurrant.com -out server.csr
